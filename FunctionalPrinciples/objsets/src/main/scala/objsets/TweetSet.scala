...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
      if (p(elem)) acc.incl(elem).union(left.filterAcc(p,new Empty)).union(right.filterAcc(p,new Empty))
      else acc.union(left.filterAcc(p,new Empty)).union(right.filterAcc(p,new Empty))
    }
  ...
  def descendingByRetweet: TweetList = {
    if (mostRetweeted.retweets>=0) new Cons(mostRetweeted,remove(mostRetweeted).descendingByRetweet)
    else Nil
  }
